...
- Event fields `specSha`/`sequenceHint`/`supersedes`/`respondsTo` — v1 keeps id, type, anchorId, quote, text, parentId, status, createdAt, schemaVersion (trigger: staleness disputes / editing semantics)
...
**Surfaces (v1):** numbered comment pins anchored to blocks AND elements within blocks · right-rail thread panel (filter chips, status pills, composer) · floating toolbar (comment-mode toggle `C`, agent-presence status) · hand-off bar with draft count · toasts for spool writes and spec reloads · changed-section badges.
