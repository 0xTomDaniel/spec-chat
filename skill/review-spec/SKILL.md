...
8. **Reconcile to empty, then select one terminal control state.** Repeat the zero-wait scan and steps 2–7 until it exits 3, then choose exactly one:
   ...
   - `host-wake`: register the resource with `scripts/review-host.py register --owner <owner-pane-id> --cursor-name .cursor-<cli-or-session>`, using the lane record owner pane id, never an agent or tab name. Every turn's re-register passes the row's current `base` from `registry.toml` as `@<row base>`, never the lane start base; an upsert takes the base it is given. Final is allowed only when registration printed `wake=verified owner=<pane>`; the review host then prompts that pane once per hand-off batch. `wake=unavailable` selects `manual-resume`.
...
Prompt-first shaping opens the HTTP page with `focus=changes&base=<exact-local-review-base>`. An explicit base is the exact selected snapshot, including a previously reviewed sibling branch; it may differ from the change request's merge base. Pin its resolved commit in the review URL. The runtime reads baseline HTML through the review server, compares stable current anchor signatures, keeps added or modified current blocks clear, and recedes unchanged current blocks. A new spec remains entirely clear. A URL with no `base` compares the registry row `base`: the last reviewed version once `review-host reviewed` has run, else the target main the lane started from. Only automatic base discovery uses a merge base. See [compared range](../../docs/specs/compared-range.spec.html#last-reviewed).
...
- **Remote browser**: use `scripts/review-host.py register` with one or more `--resource PROJECT_ID=ROOT:SPEC_PATH@BASE` entries and the lane `--slug`. It keeps one `registry.toml` with a `[process]` table and resource rows, discovers an approved port, binds `assets/review-serve.py`, and checks exact resource bytes plus `/api/baseline`. One slug may hold several roots: the first project keeps `<slug>/<spec>`, others serve at `<slug>/<project>/<spec>`, and each row records its fixed served `path`. Registering the same slug, project, and spec again is an upsert: that row takes the new root and base, its path unchanged (see [lane hosting](../../docs/specs/lane-hosting.spec.html)). Every re-register of an existing row (host-wake each turn, restart, dead server, runtime migration, recreated root) passes that row's current `base` from `registry.toml` as `@<row base>`, never the lane start base, so the [last reviewed version](../../docs/specs/compared-range.spec.html#last-reviewed) survives. Use `reviewed` on each human spec review (see the loop), `remove` to delete a resource row, and `stop` to stop the server. The URL is public and is not an authentication boundary; require no login, token, SSH, or proxy.
...
review-host stop/register, passing each row's current `base` as `@<row base>`, and
rerun host checks before handoff. This is a
