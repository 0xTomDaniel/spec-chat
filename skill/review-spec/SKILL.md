...
- **Remote browser**: use `scripts/review-host.py register` with one or more `--resource PROJECT_ID=ROOT:SPEC_PATH@BASE` entries and the lane `--slug`. It keeps one `registry.toml` with a `[process]` table and resource rows, discovers an approved port, binds `assets/review-serve.py`, and checks exact resource bytes plus `/api/baseline`. One slug may hold several roots, one project per root: the first project keeps `<slug>/<spec>`, others serve at `<slug>/<project>/<spec>`, and each row records its fixed served `path` (see [lane hosting](../../docs/specs/lane-hosting.spec.html)). Use `remove` to delete a resource row and `stop` to stop the server. The URL is public and is not an authentication boundary; require no login, token, SSH, or proxy.
