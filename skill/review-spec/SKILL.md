...
the secret URL, spec path, exact baseline, selected transport proof class, and
verification result. A remote handoff accepts either the executable probe from
a non-loopback vantage or a complete operator-confirmed public URL receipt for
the unchanged review session. A local curl or loopback request is not remote
