...
For remote or cross-machine review, block the shaping handoff until the direct public review server serves the narrow collection, never the repository root, and exact resource bytes plus the selected exact Git baseline are verified separately from transport reachability. Use approved-port discovery or explicit host configuration, bind to a free approved port, and capture the printed URL. A random port remains invalid unless it is an approved free port with collision-safe receipt. The handoff contains exactly the secret review URL, spec path, exact baseline, selected proof class, and local verification result. The URL itself is the secret; never publish it into the issue or change request. Keep the same server, URL, receipt, and checker alive through review, including empty spool, timeout, and manual-resume states. Refuse URL delivery and checker parking when neither an executable non-loopback probe nor a complete operator-confirmed public URL receipt proves transport for the unchanged session, or when exact content or baseline proof fails.
An operator-confirmed public URL receipt records proof class, UTC time, attestor, stated reviewer vantage, URL fingerprint, HTTP 200 status, resource path, exact resource hash, selected baseline commit, baseline response hash, and command or observation method. It proves only the recorded facts; visual inspection alone never claims exact byte equality. Same-machine proof records same-machine scope and proves no public reachability. `spec-chat-review` performs startup, approved-port discovery, URL capture, local byte/baseline verification, accepted transport proof, watcher/checker, wake/recovery, and shutdown; shape only enforces this gate. This interface is portable to annotateanything through its resource-specific exact-byte and baseline fields.
Failure checks are explicit: missing or non-observable hosting, unavailable approved ports, a fixed-port assumption, wrong collection root, absent or incomplete transport proof, mismatched resource bytes, or an unresolved or substituted baseline blocks handoff and means shaping cannot report ready.
