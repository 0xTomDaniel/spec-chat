...
- Tracker operations use the Linear tool named in the operating model (the `linear` CLI); Spec Chat runtime contains no tracker dependency.
...
1. Using the `linear` CLI, create a concise placeholder issue immediately, and create the work branch using target conventions. Missing issue access is a visible blocker.
...
Before finishing review, use the `linear` CLI to reconcile implementation tickets:
...
For remote or cross-machine review, block the shaping handoff until the direct public review server serves the narrow collection, never the repository root, on a free approved ingress port discovered and probed by the host. The launcher binds the selected port, verifies exact resource bytes and the selected exact Git baseline internally, and prints the secret URL only after those checks pass. The handoff contains the active secret URL, resource path, and exact baseline. Keep the same server, URL, and checker alive through review, including ordinary edits, empty spool, timeout, and manual-resume states. After each ordinary edit, rerun exact served-resource and `/api/baseline` checks for the same selected base. Restart only for a root, collection, process, port, runtime, or ownership change, or when the server is dead. Refuse handoff when approved ingress cannot be discovered, no approved port is usable, binding fails, the collection root is broad, the secret URL would enter durable public text, or exact resource or baseline proof fails. No second probe, tunnel, VPN, laptop setup, or reviewer-machine setup is required.
